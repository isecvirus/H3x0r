...
from hMorse import Morse_encode, Morse_decode
...
from hDummy import Encode_dummy, Decode_dummy, global_punctuations, arabic_punctuations
...
from hHexadecimal import Encode_Hexadecimal, Decode_Hexadecimal
from hColors import Hex2Color, Hex2RGB, Color2Hex, RGB2Hex, Color2RGB, RGB2Color
from hHtml import HTML_encode, HTML_decode
from hKMAC128 import _KMAC128_
from hKMAC256 import _KMAC256_
from hTimestamp import months_name, Date2Timestamp, Timestamp2Date
from hZlib import Zlib_compress
...
def Hexor():
    ...
    def Compare_checksum(type: str):
        ...
        if file1:
            ...
            if file2:
                ...
                if hash1 == hash2:
                    ToastNotification(title=f"{type.upper().replace('.', ' ')}",
    ...
    morse_separator_combobox = Combobox(morse_settings_frame, state='readonly', textvariable=morse_separatorVar, values=morse_separators, bootstyle='info')
    ...
    morse_space_combobox = Combobox(morse_settings_frame, state='readonly', textvariable=morse_spaceVar, values=morse_spaces, bootstyle='info')
    ...
    morse_dots_combobox = Combobox(morse_settings_frame, state='readonly', textvariable=morse_dotsVar, values=morse_dots, bootstyle='info')
    ...
    morse_dashes_combobox = Combobox(morse_settings_frame, state='readonly', textvariable=morse_dashesVar, values=morse_dashes, bootstyle='info')
    ...
    def Search(event):
        ...
        def Disable():
            ...
            Search_result_lbl.config(text="Search for type..")
        ...
        if Search_types.get():
            ...
            for r in lf:  # r=radiobutton
                if s in r['value']:
                    ...
                    # ------------------------------------
                    # Restore every type to dark bootstyle
    ...
    # --------------------------------------------------------------------------
    Search_result_lbl = Label(Search_result_frame, text="Search for type..", justify='center', anchor='center')
    ...
    execute = {
        ...
        'morse': {
            'safe': {"encode": lambda: Morse_encode(text=input.get(1.0, 'end').strip('\n'), seperator=morse_separatorVar.get(), dot=morse_dotsVar.get(), dash=morse_dashesVar.get())},
        ...
        'hexdump': {
            'safe': {"dump": lambda: HexDump(file=hexdump_file.get(), fence=hexdump_fenceVar.get(), type=hexdump_typeVar.get())},
        ...
        'shake128': {
            'safe': {"hash": lambda: Shake128(string=input.get(1.0, 'end').strip('\n').encode(), type=hashing_typeVar.get(),
                                      length=shake_lengthVar.get())},
            # 'unsafe': lambda :None,
            'checksum': {"checksum": lambda data: Shake128(string=data, type=hashing_typeVar.get(), length=shake_lengthVar.get())}
        },
        'shake256': {
            'safe': {"hash": lambda: Shake256(string=input.get(1.0, 'end').strip('\n').encode(), type=hashing_typeVar.get(),
                                      length=shake_lengthVar.get())},
            # 'unsafe': lambda :None,
            'checksum': {"checksum": lambda data: Shake256(string=data, type=hashing_typeVar.get(), length=shake_lengthVar.get())}
        },
        'blake2s': {
        ...
        'crc8': {
            'safe': {"hash": lambda: _Crc8_(string=input.get(1.0, 'end').strip('\n').encode(), type=hashing_typeVar.get())},
            # 'unsafe': lambda :None,
        ...
        'cmac': {
            'safe': {"hash": lambda: Cmac(key=cmac_key.get().encode(), string=input.get(1.0, 'end').strip('\n').encode(),
                                  type=hashing_typeVar.get())},
            # 'unsafe': lambda :None,
            'checksum': {"checksum": lambda data: Cmac(string=data, key=cmac_key.get().encode(), type=hashing_typeVar.get())}
        },
        'kmac128': {
        ...
        'poly1305': {
            'safe': {"hash": lambda: _Poly1305_(key=poly1305_key.get(), string=input.get(1.0, 'end').strip('\n'),
                                        type=hashing_typeVar.get())},
            # 'unsafe': lambda :None,
            'checksum': {"checksum": lambda data: _Poly1305_(key=poly1305_key.get(), string=data, type=hashing_typeVar.get())}
        },
        'whirlpool': {
        ...
        'aes128': {
            ...
            'unsafe': {"decrypt": lambda: AES128_Decryption(data=input.get(1.0, 'end').strip('\n'), key=aes128_key.get())},
            'checksum': {"checksum": lambda data: AES128_Encryption(data=data)}
        },
        'punycode': {
            'safe': {"encode": lambda: Punycode.encode(string=input.get(1.0, 'end').strip('\n'))},
            'unsafe': {"decode": lambda: Punycode.decode(string=input.get(1.0, 'end').strip('\n'))}
        },
        'hexadecimal': {
        ...
        'html': {
            'safe': {"encode": lambda: HTML_encode(string=input.get(1.0, 'end').strip('\n'), quote=HTML_quoteVar.get())},
            'unsafe': {"decode": lambda: HTML_decode(string=input.get(1.0, 'end').strip('\n'))}
        },
        'timestamp': {
            'safe': {"timestamp": lambda: Date2Timestamp(year=timestamp_yearVar.get(), month=timestamp_month_numberVar.get(),
                                            day=timestamp_dayVar.get(), hour=timestamp_hourVar.get(),
                                            minute=timestamp_minuteVar.get(), second=timestamp_secondVar.get())},
        ...
        'replace': {
            'safe': {"replace": lambda: input.get(1.0, 'end').strip('\n').replace(replace_from.get(), replace_to.get())},
            # 'unsafe': lambda: Timestamp2Date(timestamp=input.get(1.0, 'end').strip("\n"))
        },
        'remove': {
            'safe': {"remove": lambda: input.get(1.0, 'end').strip('\n').strip(remove_character.get())},
            # 'unsafe': lambda: Timestamp2Date(timestamp=input.get(1.0, 'end').strip("\n"))
        }
        ...
        if selected_typeVar.get():
            ...
            if method in execute[type]:
                if method in methods:
                    try:
                        _out_ = execute[type][method][action]()
                        output.delete(1.0, 'end')
                    ...
                    if file:
                        try:
                            _out_ = execute[type][method][action](open(file=file, mode="rb").read())
                            input.delete(1.0, 'end')
    ...
    move_output2input = Button(Bottom_WorkSpace_actions_Frame, text="▲", padding=10, bootstyle='secondary', cursor='hand2',
                               takefocus=False, command=lambda: Output2Input())
    move_output2input.pack(side='left', fill='x', expand=True)
    # -------------------------------------------------------------------
    Safe = Button(Top_WorkSpace_actions_Frame, text="Safe", bootstyle="success", padding=10, cursor='hand2', takefocus=False)
    Safe.pack(side='left', fill='x', expand=True)
    ...
    move_input2output = Button(Bottom_WorkSpace_actions_Frame, text="▼", padding=10, bootstyle='secondary', cursor='hand2',
    ...
    UnSafe.pack(side='right', fill='x', expand=True)
    # -------------------------------------------------------------------
    Checksum = Button(Top_WorkSpace_actions_Frame, text="Checksum", bootstyle="primary", padding=10, cursor='hand2', takefocus=False)
    Checksum.pack(side='left', fill='x', expand=True)
    ####################################################################
